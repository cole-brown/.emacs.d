...
(defcustom mis2/parts/type-list
...
(defcustom mis2/parts/symbols-alist
  '(;;---
...
;; (alist-get :newline mis2/parts/symbols-alist)
;; (alist-get :string-full mis2/parts/symbols-alist)
...
See `mis2/parts/type-list' for list.

Returns an element from `mis2/parts/type-list'."
  (cond
   ((symbolp part)
    :symbol)
   ...
   ;; Dunno. invalid
   (t
    (mis2/warning nil :warning
                 "Don't know how to classify type of part: '%S'"
                 part)
    :invalid)))


(defun mis2/parts/property (type part)
...
(defun mis2/parts/value (type part)
  ...
  (let ((type (mis2/parts/type section)))
    (cond
     ...
     ((eq type :format)
      ;; No more parts to this section.
      nil))))
;; (mis2/parts/next '(:indent "  " :border "++" :padding "=="))
...
(defun mis2/parts/process/symbol (symbol)
  "Processes a SYMBOL into its parts."
  (if-let ((value (alist-get symbol mis2/parts/symbols-alist)))
      (cond
       ...
       ((functionp (nth 0 value))
        ...
        ;; didn't want to eval symbols in `mis2/parts/symbols-alist' then,
    ...
    (mis2/warning
     nil :warning
     "No symbol in `mis2/parts/symbols-alist' for: '%S' '%S'"
     symbol mis2/parts/symbols-alist)))
;; (mis2/parts/process/symbol :newline)
;; (mis2/parts/process/symbol :line-full)
...
Returns for:
  ...
  (let ((result nil)
        (type (mis2/parts/type part)))
    (cond
     ;;---
     ;; type == invalid
     ;;---
     ((eq type :invalid)
      ;; `mis2/parts/type' already complained - just pass nil as part.
      nil)
     ...
     ;;---
     ((eq type :symbol)
      (setq result (mis2/parts/process/symbol part)))
    ...
    result))
;; (mis2/parts/process :line-empty)
;; (mis2/parts/process :string-empty)
;; (mis2/parts/process :string-full)
;; (mis2/parts/process "hello")
;; (mis2/parts/process '(:padding "----"))
;; (mis2/parts/process '(:text "Hello, %s %s %s" "a" "b" "c!"))
;; (mis2/parts/process '(:indent "  " :border "++" :padding "=="))


(defun mis2/parts/propertize (string prop &optional faces-plist)
  ...
  (let ((face-val (plist-get faces-plist prop)))
    (if face-val
      ...
      string)))
;; (mis2/parts/propertize "    " :indent nil)


(defun mis2/parts/section (section &optional faces-plist)
  "Builds a SECTION of a string parts list."
  (let* ((type     (mis2/parts/type section))
         (prop     (mis2/parts/property type section))
         (value    (mis2/parts/value type section))
    ...
    ;; it'll be a string...
    (apply #'mis2/parts/propertize
           ;; either now...
           (if (stringp value)
               (list value prop faces-plist)
             ;; ...or after going deeper.
             (list
              (mis2/parts/build value
                               faces-plist)
              prop
              faces-plist)))))
...
(defun mis2/parts/build (arg &optional faces-plist)
...
§-TODO-§ [2019-10-22]: section here about what all arg can be."
  ...
  (if (or
    ...
    ;; sections. >.>
    (let ((accum nil)
          (input (mis2/parts/process arg)))
      ;; push to accum until no more sections
      (while input
        ;; process first section of input
        (push (mis2/parts/section input
                                 faces-plist)
              accum)

        ;; update loop conditional
        (setq input (mis2/parts/next input)))

      ;; put 'em back in correct order and stringify 'em.
      (mapconcat #'identity (nreverse accum) ""))))
;; (mis2/parts/build '(:border "||" :padding "----"))
;; (mis2/parts/build '(:prefix
;;                    (:indent "" :border "||" :padding "--")
;;                    :center
;;                    (:text "xx")
;;                    :postfix
;;                    (:padding "==" :border "!!")))
;; (mis2/parts/build :string-empty)
;; (mis2/parts/build :string-full)
;; (mis2/parts/build "hello")
;; (mis2/parts/build '(:text "Hello, %s %s %s" "a" "b" "c!"))
;; (mis2/parts/build '(:indent "  " :border "++" :padding "=="))
;; (mis2/parts/build '(:john (:title "Test?") :text (:title " %s %s %s" "a" "b" "c!")))

...
(defcustom mis/comment/border-adjustments
...
;; (mis/comment/center/build "(will center on exit)" nil mis/center/char/padding)
...
If TRIM is non-nil, trims resultant string before returning."
  (mis/comment/unless
    (let* ((string (if (stringp arg) arg (format "%s" arg)))
           ...
           (comment-parts (mis/comment/borders border-adj))
...
(defun mis/comment/borders (&optional with-adjustments)
  ...
  (mis/comment/unless
    (let* ((adjustment
           ...
           (prefix   (string-trim-right (comment-padright comment-start
                                                          pad-more)))
           (postfix  (string-trim-left (comment-padleft comment-end pad-more)))
      ...
      (list prefix postfix))))
;; (mis/comment/borders)
;; (nth 0 (mis/comment/borders))
;; (nth 1 (mis/comment/borders))
;; (length (nth 1 (mis/comment/borders)))
...
borders (comment delimiters) assuming nil BORDERS."
  (mis/comment/unless
    (let* ((borders (or borders (mis/comment/borders t))))
